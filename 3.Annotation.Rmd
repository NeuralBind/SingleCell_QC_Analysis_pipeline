...
# 
# p<- DimPlot(Sc_filtered, reduction = "umap", label = T, group.by = "SCT_snn_res.0.08", alpha = 0.14)
# 
# p
...
ggsave("SCT/cell_type_conditions_stackedXUE_indiv.pdf", stacked, width = 10, height = 8)
```

```{r}

#Sc_filtered$cell_type <- Sc_filtered$

#Sc_filtered <- readRDS("SCT/annotated_intgr_res0_08.rds")
...
p1 <- ggplot(cell_perc, aes(x = condition, y = mean_percentage, fill = condition)) +
      ...
      theme_minimal()
p1
ggsave("output/cell_type_anch.pdf", p1, width = 10, height = 8)
```
