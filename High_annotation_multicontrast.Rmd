...
#And the level of mph
myeloid_cells <- c("Mph", "Mono", "KCs", "MDMs")
metadata_df$cell_type[metadata_df$cell_type %in% myeloid_cells] <- "Mph"
...
save = TRUE
if(save == TRUE){
  saveRDS(multinichenet_output, paste0(path, "multiContrast_RNA_HighLevel.rds"))
}
```
...
ggsave("multiniche_output/multicontrast_all_cells/output/pretumor_top_LR.pdf", plot_oi, width = 20, height = 30, dpi = 300)
...
ggsave("multiniche_output/multicontrast_all_cells/output/tumor_top_LR.pdf", plot_oi, width = 20, height = 30, dpi = 300)
```
...
cell_list <- as.list(unique(sce$cell_type))
...
for (receiver_oi in cell_list) {
  ...
  file_name = paste0("multiniche_output/multicontrast_all_cells/output_receivers/",group_oi,"_receivers/",group_oi,"_top_70_", receiver_oi, "_receivers.pdf")
...
for (sender_oi in cell_list) {
  ...
  file_name <- paste0("multiniche_output/multicontrast_all_cells/output_senders/",group_oi,"_senders/",group_oi,"_top_70_", sender_oi, "_senders.pdf")
  ggsave(file_name, plot_oi, width = 15, height = 10, dpi = 300)
}
```
...
for (receiver_oi in cell_list) {
  ...
  tryCatch({
    ...
    file_name = paste0("multiniche_output/multicontrast_all_cells/output/",group_oi,"_ligand_receiver", receiver_oi, "_ligand_target_bars.pdf")
