...
DimPlot(object, reduction = "umap", label = TRUE, shuffle = T, group.by = "cell_subtype")
...
write.csv(da_T_ctl_int, "T_da_ctlvsint.csv", row.names = FALSE)
write.csv(da_T_int_end, "T_da_endvsint.csv", row.names = FALSE)

write.csv(da_Myelo_ctl_int, "Myelo_da_ctlvsint.csv", row.names = FALSE)
write.csv(da_Myelo_int_end, "Myelo_da_endvsint.csv", row.names = FALSE)

write.csv(da_Ec_ctl_int, "Ec_da_ctlvsint.csv", row.names = FALSE)
write.csv(da_Ec_int_end, "Ec_da_endvsint.csv", row.names = FALSE)
```
...
T2 <- plotUMAP(T_milo, colour_by="cell_subtype", point_size=0.9, text_by = "cell_subtype", text_size = 3.5) + plotNhoodGraphDA(T_milo, da_T_int_end, alpha=0.5) +
      plot_layout(guides="auto" )+
      ggtitle("Intermediate vs End Tcells ")


ggsave("Milo_T_ct_to_int.pdf", plot = T1, device = "pdf", dpi = 300, width = 10, height = 7)
ggsave("Milo_T_int_to_end.pdf", plot = T2, device = "pdf", dpi = 300, width = 10, height = 7)
```
...
Ec2 <- plotUMAP(Ec_milo, colour_by="cell_subtype", point_size=0.9, text_by = "cell_subtype", text_size = 3.5) + plotNhoodGraphDA(Ec_milo, da_Ec_int_end, alpha=0.5) +
      plot_layout(guides="auto" )+
      ggtitle("Intermediate vs End Ecs")

ggsave("Milo_Ec_ct_to_int.pdf", plot = Ec1, device = "pdf", dpi = 300, width = 10, height = 7)
ggsave("Milo_Ec_int_to_end.pdf", plot = Ec2, device = "pdf", dpi = 300, width = 10, height = 7)
```
...
M2 <- plotUMAP(Myelo_milo, colour_by="cell_subtype", point_size=0.9, text_by = "cell_subtype", text_size = 3.5) + plotNhoodGraphDA(Myelo_milo, da_Myelo_int_end, alpha=0.5) +
      plot_layout(guides="auto" )+
      ggtitle("Intermediate vs End Monocytes")

ggsave("Milo_Mono_ct_to_int.pdf", plot = M1, device = "pdf", dpi = 300, width = 10, height = 7)
ggsave("Milo_Mono_int_to_end.pdf", plot = M2, device = "pdf", dpi = 300, width = 10, height = 7)
```

```{r}
da_T_ctl_int <- annotateNhoods(T_milo, da_T_int_end, coldata_col = "cell_subtype")
da_T_int_end <- annotateNhoods(T_milo, da_T_int_end, coldata_col = "cell_subtype")

da_Ec_ctl_int <- annotateNhoods(Ec_milo, da_Ec_int_end, coldata_col = "cell_subtype")
...
da_Myelo_ctl_int$cell_subtype <- ifelse(da_Myelo_ctl_int$cell_subtype_fraction < 0.7, "Mixed", da_Ec_ctl_int$cell_subtype)
