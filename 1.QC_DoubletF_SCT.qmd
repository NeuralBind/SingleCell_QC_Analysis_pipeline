...
## Qc filteringundefined # this is a test to check quartiles (dont use yet, just observe the quartiles)
...
for (object_name in RasV_stages.list) {
  ...
  upper_lim_nCount <- quantile(nCount_values, 0.98)
  
  # Lower limit quantile
  lower_lim_nCount <- quantile(nCount_values, 0.02)
...
for (name in RasV_stages.list) {
  current_object <- get(name)
  print(name)
  sweep.list <- paramSweep(current_object, PCs= 1:50, sct = TRUE) ###!!!!!!!!!!! CHANGE THSI IF YOU RUN SCT also when run doublet finder!!!!!!!!!!!!#######
  ...
  current_object <- doubletFinder(current_object, PCs = 1:50, pN = 0.25, pK = pK, nExp = nExp_poi, reuse.pANN = FALSE, sct = TRUE ) ######## CHANGE IF YOU RUN SCT BEFORE!!!!!!!!!!!!######
  # select column name in reuse.pan
  current_object <- doubletFinder(current_object, PCs = 1:50, pN = 0.25, pK = pK, nExp = nExp_poi.adj, reuse.pANN = colnames(current_object@meta.data)[11], sct = TRUE) 
...
## If i want to remove the doublets from now, I saw how to remove them also the pan column,1st classification  in data intergration script
...
for (name in RasV_stages.list) {
  ...
  filepath <- paste0(dir_path, name, "_1stfilt_SCT.rds")
  saveRDS(current_object, file = filepath)
}
```
