...
```{r}
Sc_filtered <- readRDS(file ="output/Sc_annotated.rds")
```
...
```{r}
DimPlot(Sc_filtered, reduction = "umap", group.by = "predicted_label")
```
...
Sc_filtered$cell_type_gen <- gsub("_.*", "", Sc_filtered$predicted_label)
...
sender_celltypes = c("CD4T","Mph", "Mo", "NK", "B", "DC", "Mono-like")
...
ctive_ligand_target_links_df = best_upstream_ligands %>% lapply(get_weighted_ligand_target_links,geneset = geneset_oi, ligand_target_matrix = ligand_target_matrix, n = 200) %>% bind_rows() %>% drop_na()
